...
+ (void)cropVideoWithAsset:(AVURLAsset *)asset
    ...
    [self jp_cropVideoWithAsset:asset
                  presetName:presetName
                   configure:configure
                    cacheURL:cacheURL
                  errorBlock:errorBlock
...
static AVAssetReader *assetReader_;
static AVAssetWriter *assetWriter_;
+ (void)jp_cropVideoWithAsset:(AVURLAsset *)asset
                presetName:(NSString *)presetName
                 configure:(JPCropConfigure)configure
                  cacheURL:(NSURL *)cacheURL
                errorBlock:(JPImageresizerErrorBlock)errorBlock
    ...
    AVAssetReader *assetReader = [[AVAssetReader alloc] initWithAsset:asset error:&readerError];
    AVAssetWriter *assetWriter = [[AVAssetWriter alloc] initWithURL:cacheURL fileType:fileType error:nil];
    ...
    AVAssetTrack *videoTrack = [[asset tracksWithMediaType:AVMediaTypeVideo] firstObject];
        ...
        // 设置缩放和位移
        CGAffineTransform transform = CGAffineTransformIdentity;
        transform = CGAffineTransformScale(transform, 0.5, 0.5); // 缩放为原来的一半
        transform = CGAffineTransformTranslate(transform, 320, 180); // 位移
        [layerInstruction setTransform:transform atTime:kCMTimeZero];
        
        instruction.layerInstructions = @[layerInstruction];
        videoComposition.instructions = @[instruction];
        
        // 创建 AVAssetReaderVideoCompositionOutput
        NSDictionary *readerOutputSettings = @{(id)kCVPixelBufferPixelFormatTypeKey: @(kCVPixelFormatType_32BGRA)};
        AVAssetReaderVideoCompositionOutput *videoCompositionOutput = [AVAssetReaderVideoCompositionOutput assetReaderVideoCompositionOutputWithVideoTracks:@[videoTrack] videoSettings:readerOutputSettings];
    ...
    NSDictionary *writerOutputSettings = @{
        ...
        AVVideoCompressionPropertiesKey: @{
            AVVideoAverageBitRateKey: @2000000, // 比特率设置
            AVVideoProfileLevelKey: AVVideoProfileLevelH264High40,
        }
    };
    AVAssetWriterInput *assetWriterInput = [AVAssetWriterInput assetWriterInputWithMediaType:AVMediaTypeVideo outputSettings:writerOutputSettings];
            ...
            if (sampleBuffer) {
                ...
                [assetWriter finishWritingWithCompletionHandler:^{
                    ...
                    if (assetWriter.status == AVAssetWriterStatusCompleted) {
                        ...
                        [[NSFileManager defaultManager] removeItemAtURL:tmpURL error:nil];
                        [self __executeErrorBlock:errorBlock cacheURL:cacheURL reason:(assetWriter.status == AVAssetWriterStatusCancelled ?  JPIEReason_VideoExportCancelled : JPIEReason_VideoExportFailed)];
                    }
